...
console.log("4");
  	  ...
  	  socket.on('login', function (data) { console.log("5"); console.log(data);
	  ...
	  socket.on('new message', function (data) { console.log("6"); console.log(data);
	  ...
	  socket.on('user joined', function (data) { console.log("7"); console.log(data);
	  	addMessageToList("",false,data.username + " joined")
	  	addMessageToList("",false,message_string(data.numUsers)) 
	  });

	  // Whenever the server emits 'user left', log it in the chat body
	  socket.on('user left', function (data) { console.log("8");
	    addMessageToList("",false,data.username+" left")
	    addMessageToList("",false,message_string(data.numUsers))
	  });

	  //Whenever the server emits 'typing', show the typing message
	  socket.on('typing', function (data) { console.log("9"); console.log(data);
	    addChatTyping(data);
	  });

	  // Whenever the server emits 'stop typing', kill the typing message
	  socket.on('stop typing', function (data) { console.log("9");
	    removeChatTyping(data.username);
	  });	
  	})

  	//function called when user hits the send button
  	self.sendMessage=function(){ console.log("10");
  	...
  	self.updateTyping=function(){ console.log("11");
  		sendUpdateTyping()
  	}

  	// Display message by adding it to the message list
  	function addMessageToList(username,style_type,message){ console.log("12");
  	...
  	function getUsernameColor (username) { console.log("13");
	    // Compute hash code
	    var hash = 7;
  	...
  	function sendUpdateTyping(){ console.log("14");
  		if(connected){
  			if (!typing) {
		        typing = true;
		        socket.emit('typing');
	...
	function addChatTyping (data) { console.log("15");
	    addMessageToList(data.username,true," is typing");
	}

	// Removes the visual chat typing message
	function removeChatTyping (username) { console.log("16");
